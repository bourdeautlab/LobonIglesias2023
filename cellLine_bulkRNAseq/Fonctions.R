...
format_GO_labels <- function(x, prefix = prefix, wordsLength = 4,...)
{
  gsub(pattern = paste0("((?:[^_]+_){",wordsLength,"}[^_]+)"), replacement = "\\1\n", x) %>%
    gsub(pattern = prefix, replacement = "", x) %>%
    gsub(pattern = "_", replacement = " ", x)
}

# Plot GO overlap result
plot_top_MSigDB_GO_enriched_geneSets_func <- function(MSigDB_GO_excel_result,
                                                      top               = 10,
                                                      title             = NULL,
                                                      xlim              = 400,
                                                      sep               = ";",
...
)
{
  ...
  GO_res_lines <- readLines(MSigDB_GO_excel_result)
  
  # Select pathway table result
  top_enriched_GO_pathway <- grep(pattern = "^GO_*",
                                  x       = GO_res_lines
  )
  
  GO_res_table <- read.table(text      = GO_res_lines[top_enriched_GO_pathway],
                             header    = FALSE,
                             sep       = sep,
                             quote     = quote
  )
  
  GO_res_table <- GO_res_table[,1:7]
  colnames(GO_res_table) = c("pathway_ID","K","pathway_name","k","k_K","pvalue","FDR")
  #GO_res_table
  
  
  # Order pathway name according to FDR 
  GO_res_table$pathway_ID <- factor(x      = GO_res_table$pathway_ID,
                                    levels = GO_res_table$pathway_ID[order(GO_res_table$FDR,
                                                                           decreasing = TRUE
                                    )
                                    ]
  )
  
  if (nrow(GO_res_table) < top)
  {
    top <- nrow(GO_res_table)
  }
  
  # Select top genes
  GO_res_table_top <- GO_res_table[1:top,]
  
  # Give different color and face for selected gene set names
  GO_res_table_top$y_text_color <- ifelse(GO_res_table_top$pathway_ID %in% selected_geneSet,
                                          "black",
                                          "black"
  )
  GO_res_table_top$y_text_face  <- ifelse(GO_res_table_top$pathway_ID %in% selected_geneSet,
                                          "bold",
                                          "plain"
  )
  ...
  if (format_labels)
  {
    GO_res_table_top$pathway_ID <- format_GO_labels(GO_res_table_top$pathway_ID,
  ...
  # Plot barplot
  g <- ggplot(data    = GO_res_table_top,
    ...
    ylim(0,xlim) +
    geom_text(mapping = aes(label=paste0("(",k,"/",K,")")), size = fontsize/2, hjust = -0.1) +
    theme_bw() +
    theme(title = element_text(color = "black", size = fontsize),
          ...
          axis.text.y  = element_text(color = rev(GO_res_table_top$y_text_color),
                                      face  = rev(GO_res_table_top$y_text_face),
                                      size  = label_fontsize
          )
    )
  return(g)    
...
)
{
  ...
  g <- ggplot(data    = res_table_top,
    ...
    geom_text(mapping = aes(label=paste0("(",k,"/",K,")")), size = fontsize/2, hjust = -0.1) +
    theme_bw() +
    theme(title = element_text(color = "black", size = fontsize*1.5),
          panel.border = element_blank(),
          axis.text.x  = element_text(color = "black", size = fontsize*1.4),
          axis.title.x = element_text(color = "black", size = fontsize*1.5),
